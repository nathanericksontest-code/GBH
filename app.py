...
def run_zapier(df_zap,df_prepack,destination):
    ...
    if selected_bag_to_edit is not None:
        ...
        with st.form(f"{destination}_form"):
            ...
            lot_cols = [c for c in TICKET_COLUMNS if "lot" in c.lower()]
            other_cols = [c for c in TICKET_COLUMNS if "lot" not in c.lower()]
...
if not is_authenticated:
    ...
    if not df_raw.empty:
        ...
        # Change price string
        df_raw["$ Price"] = df_raw["Price"].copy()
        df_raw["$ Price"] = (df_raw["$ Price"].astype(str).str.replace("$", "", regex=False).str.replace(",", "", regex=False).str.strip())
        df_raw["$ Price"] = pd.to_numeric(df_raw["$ Price"])

        # 3. Apply your cash payment mapping condition
        df_raw["Cash"] = np.where(df_raw["Payment source"] == "Cash payment", df_raw["$ Price"], 0.0)
    ...
    if page_selection == "📋 Live Transaction Ledger":
            ...
            if not is_authenticated:
                ...
                display_cols = ["ID", "Status", "Attendee first name", "Attendee last name", "Ticket name", "Broad Category Group", "Check-in time", "Check-in by","Refunded by","Refund date","Cash"]
